...
def iterate_pagerank(corpus, damping_factor):
    ...
    PageRank values should sum to 1.
    """
    N =len(corpus) # 语料库中页面总数
    pagerank = dict.fromkeys(corpus.keys(),1/N) #初始化字典
    numlinks = {p:len(corpus[p]) for p in corpus.keys()}# 计算每个页面的链接数量
    flag = True
    while flag:
        flag = False
        newrank = {}
        for p in pagerank.keys():
            ...
            if abs(newrank[p] - pagerank[p]) > 1e-3:
                flag = True # 如果pr值变大就继续迭代
        for p in pagerank.keys():
            pagerank[p] = newrank[p] # 更新pr值
