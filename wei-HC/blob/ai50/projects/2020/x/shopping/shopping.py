...
def load_data(filename):
    ...
    with open(filename) as f:
        ...
        for row in reader:
            ...
            evidence.append(data)# 将单个样本添加到evidence
...
def train_model(evidence, labels):
    ...
    """
    model = KNeighborsClassifier(1) # 创建k临分类器
    model.fit(evidence,labels) # 在数据集上训练模型
    return model
...
def evaluate(labels, predictions):
    ...
    sensitivity = tp / totp # 计算敏感性
    specificity = tn / totn # 计算特异性
